    ...
    <Card className="w-full max-w-xl rounded-[2rem] border border-amber-200 bg-amber-50/90 shadow-xl shadow-amber-950/5">
      <CardHeader className="space-y-3">
        <CardTitle className="font-display text-3xl font-semibold text-slate-950">
          {showTechnicalDetails ? "Clerk setup is still required" : "Sign-in is not available yet"}
        </CardTitle>
        <CardDescription className="text-base text-slate-700">
          {showTechnicalDetails
            ? "Google sign-in is wired into the app, but the Clerk publishable key and secret key have not been added to this environment yet."
            : "Google sign-in will appear automatically once the production services finish connecting."}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 text-sm text-slate-700">
        {showTechnicalDetails ? (
          ...
          <>
            <p>The public app is live, but authentication is still being finalized.</p>
            <p>
              You can return to the{" "}
              <Link href="/" className="font-semibold text-cyan-800 hover:text-cyan-900">
                main GarageFlow page
              </Link>
              {" "}while setup completes.
            </p>
          </>
        )}
      </CardContent>
